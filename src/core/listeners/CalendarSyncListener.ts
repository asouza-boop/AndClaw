...
async function syncTaskToCalendar(payload: any) {
  ...
  let storedEventId = Boolean(gcalEventId);
  ...
  for (const account of accounts) {
    ...
    try {
      if (gcalEventId) {
        ...
        if (newEventId && !storedEventId) {
          await query(`UPDATE tasks SET gcal_event_id = $1 WHERE id = $2`, [newEventId, payload.taskId]);
          storedEventId = true;
        }
      }
      logger.info('calendar.sync.task_synced', { taskId: payload.taskId, account: account.email });
    } catch (e: any) {
      logger.error('calendar.sync.task_account_failed', { taskId: payload.taskId, account: account.email, error: e.message });
    }
  }
}
...
async function syncMeetingToCalendar(payload: any) {
  ...
  for (const account of accounts) {
    ...
    try {
      ...
      logger.info('calendar.sync.meeting_synced', { meetingId: payload.meetingId, account: account.email });
    } catch (e: any) {
      logger.error('calendar.sync.meeting_account_failed', { meetingId: payload.meetingId, account: account.email, error: e.message });
    }
  }
}
